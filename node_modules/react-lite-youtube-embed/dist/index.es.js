...
const q = {
  ...
  const [a, r] = G("");
  return z(() => {
    const o = `https://img.youtube.com/${t}/${e}/${s}.${u}`, h = `https://img.youtube.com/${t}/${e}/hqdefault.${u}`, c = q[s], d = new Image();
    d.onload = () => {
      d.width < c ? r(h) : r(o);
...
function ee(e, t) {
  const [u, s] = l.useState(!1), [a, r] = l.useState(e.alwaysLoadIframe || !1), o = encodeURIComponent(e.id), h = typeof e.playlistCoverId == "string" ? encodeURIComponent(e.playlistCoverId) : null, c = e.title, d = e.poster || "hqdefault", $ = e.announce || "Watch", U = e.alwaysLoadIframe ? e.autoplay && e.muted : !0, C = l.useMemo(() => {
  ...
  return l.useEffect(() => {
    ...
    const L = [], N = () => {
      ...
      if (w)
        return;
      w = !0, N(), [100, 300, 600, 1200, 2400].forEach((n) => {
    ...
    return typeof t == "object" && t?.current ? (t.current.addEventListener("load", T), t.current.contentDocument?.readyState === "complete" && T()) : [200, 500, 1e3, 2e3, 3e3].forEach((n) => {
      ...
      window.removeEventListener("message", k), L.forEach(clearTimeout), typeof t == "object" && t?.current && t.current.removeEventListener("load", T);
    };
  }, [
    a,
    e.enableJsApi,
